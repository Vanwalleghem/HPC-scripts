...
img_seq_list=glob.glob(tif_file_folder+'/3Dreg/*.nii')

sizes=np.zeros(len(img_seq_list))
for i,img_name in enumerate(img_seq_list):
    sizes[i]=os.stat(img_name).st_size
temp=[img_seq_list[j].replace('_LongReg.nii','.tif') for j in np.where(sizes<sizes.max())[0].astype(int)]
...
frames=np.zeros((len(img_seq_list)*base_img.shape[2],base_img.shape[1],base_img.shape[0]), dtype='uint32')
for img_nb,C1_name in enumerate(img_seq_list):
    print(img_nb)
