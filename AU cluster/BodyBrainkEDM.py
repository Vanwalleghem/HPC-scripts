...
#exp_list=[x.split('experiment')[1].split(os.sep)[0] for x in glob.glob(os.path.join(download_dir,'experiment*/'))]
#exp_list= [4,5]
exp_list= [5]
#exp_list.sort()
...
for exp_no in exp_list:    
    ...
    sub_list.sort()
    for sub in sub_list:
        ses_list = [x.split('ses-')[1].split(os.sep)[0] for x in glob.glob(os.path.join(base_dir,'sub-'+str(sub),'ses-*/'))]
        for ses in ses_list:
                ...
                if not stim_list:
                    ...
                    for stim in stim_list:
                        if os.path.exists(os.path.join(download_dir,'results','experiment'+str(exp_no)+rf'sub-{sub}_ses-{ses}_task-stim{stim}_ccm_rho_Brainbody.npy')):
                            ...
                            np.save(os.path.join(download_dir,'results','experiment'+str(exp_no)+rf'sub-{sub}_ses-{ses}_task-stim{stim}_Embed_dims.npy'),Embed_dims)
                            ccm_rho=kedm.xmap(timeseries,Embed_dims,5,1)
                            np.save(os.path.join(download_dir,'results','experiment'+str(exp_no)+rf'sub-{sub}_ses-{ses}_task-stim{stim}_xmap.npy'),ccm_rho)
                            ...
                            np.save(os.path.join(download_dir,'results','experiment'+str(exp_no)+rf'sub-{sub}_ses-{ses}_task-stim{stim}_ccm_rho_bodyBrain.npy'),CCM_array_body_brain)
                            np.save(os.path.join(download_dir,'results','experiment'+str(exp_no)+rf'sub-{sub}_ses-{ses}_task-stim{stim}_ccm_rho_Brainbody.npy'),CCM_array_brain_body)