...
file_to_check='ToCheckWarp.txt'

with open(file_to_check) as f:
 for line in f: 
  line = line.strip() #or some other preprocessing
  if not "MAX" in line:
   fnames_all.append(line.split('/3Dreg')[0]) #storing everything in memory!
...
with open('Convert_array.sh','w') as the_file:
 ...
 job_string = "filename=`sed -n ${SLURM_ARRAY_TASK_ID}p ToCheckWarp.txt` \n"
 the_file.write(job_string) 
 ...
 job_string = 'python ~/Greedy_MC_3Dstack_Combined.py $filename \n' 
 the_file.write(job_string)
