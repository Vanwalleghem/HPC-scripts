...
for img_name in img_seq_list:
  if not os.path.exists(img_name.replace('Greedy.nii','CombinedWarp.nii.gz')):
    output_name=img_name.replace('Greedy.nii','CombinedWarp.nii.gz')    
    #Combine warps into one warp
    #job_string = "antsApplyTransforms -d 3 -o ["+output_name+",1] -t "+img_name.replace('_Greedy.nii','.tif_Greedy2.nii')+" -t "+img_name+" -r "+template_file
    #call([job_string],shell=True)  
    if not os.path.exists(img_name.replace('Greedy.nii','CombinedWarp.nii.gz')): #if it fails, it usually is because of Greedy.nii, so we recompute it
        ...
        Mov_name=img_name.replace('_Greedy.nii','.tif')        
        ...
        job_string = job_string.replace('Affine_name',Affine_name).replace('OutImg',img_name).replace('FixImg',template_name).replace('MovImg',Mov_name).replace('MaskImg',Mask_name)
        call([job_string],shell=True)          
    #Jacob_name = img_name.split('_Greedy.nii')[0]+'_Jacobian2.nii'
